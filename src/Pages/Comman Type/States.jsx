...
import AddStatepopup from "../../Components/AddStatepopup";
import Pagination from "../../Components/Pagination";
import { deletestate } from "../../utils/api/Stateapi/Index";
...
function States() {
  ...
  const onepage = state.slice(firstindex, lastindex);
  ...
  useEffect(() => {
    getstate(id).then((res) => setstate(res));
  }, [id,flag]);
  ...
  const handledelete=(id)=>(
    deletestate(id).then(res=>{if(res=="success"){
      setflag(!flag)
  ...
  const handlechange = (e) => {
    ...
    setallsearcheddata(
      state.filter((item) => {
        return item.StateName.toLowerCase().includes(e.target.value);
      })
    );
  };
  ...
  return (
    <div className="m-8">
      ...
      <div className="flex  items-center justify-between ">
        <div className=" text-3xl font-semibold text-[#680d0d]">States</div>
        <div>
          <div className=" text-2xl  text-[#680d0d]">
            {<AddStatepopup name="add" flag={flag} setflag={setflag} />}
          </div>
        </div>
      </div>
      ...
      {id ? (
          ...
          <table className="w-full ">
              ...
              >
                <div>
                  <th className="text-left justify-start flex gap-2 items-center h-12  w-full  ">
                    <div className=" pl-5">State List</div>
            ...
            <tbody>
              {(allsearchdata[0] ? searchonepage : onepage)
                // .filter((item)=>{
                //   return search === '' ? item : item.StateName.toLowerCase().includes(search)
                // })
                .map((s) => (
                  <tr className="border h-12 hover:bg-gray-100">
                    <td className="px-5">{s.StateName}</td>
                    <td className="flex justify-end mt-3 px-5  text-2xl">
                      {<AddStatepopup name="edit" flag={flag} setflag={setflag} id= {s.StateId} sname={s.StateName} cname={s.CountryId} />}
                      {<MdDelete className="text-orange-600" onClick={()=>{handledelete(s.StateId)}}/>}
                    </td>
                  </tr>
                ))}
            </tbody>
        ...
        <div className="w-full border border-black mt-4 bg-gray-100 h-[500px] flex justify-center ">
          <p className="mt-10 text-xl font-bold">Please Select State</p>
        </div>
      )}
    </div>
  );
