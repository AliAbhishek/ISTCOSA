...
const AllUser = (props) => {
  ...
  useEffect(() => {
    getregistereduser("alluser", page).then((res) => setalluser(res));
  }, [flag, page]);
    ...
    <div>
      <div className="overflow-auto h-[400px] mt-6 border">
        <div className="mx-6 flex flex-col md:flex-row  justify-between h-12 text-sm">
          ...
          <div className="flex  items-center gap-1">
            <UserPagination
              pages={alluser[0]?.PageSize}
              changepage={changepage}
              page={page}
              setpage={setpage}
              TotalPageCount={alluser[0]?.TotalPageCount}
        ...
        <table className="w-full  ">
          ...
          <tbody className="overflow-y-scroll no-scrollbar lg:top-0  md:top-14      ">
            {(props.filteredresult[0]?props.filteredresult:alluser)
              // .filter((item) => {
