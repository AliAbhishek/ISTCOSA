...
function Notregistered(props) {
  ...
  useEffect(() => {
    getregistereduser("nonRegistered", page).then((res) => setNotRegistered(res));
  }, [flag,page]);
      ...
      {/* {user == "Not-Registered" ? ( */}
        <div className="overflow-auto h-[400px] mt-6 border">
          <div className=" mx-6 flex flex-col md:flex-row  justify-between h-12 text-sm">
            ...
            <div className="flex  items-center gap-1"> <UserPagination

              pages={notRegistered[0]?.PageSize}
              changepage={changepage}
              page={page}
              setpage={setpage}
              TotalPageCount={notRegistered[0]?.TotalPageCount}
              registeredUser={notRegistered}
            /></div>
          </div>
          <table className="w-full  ">
            ...
            {(props.filteredresult[0]?props.filteredresult:notRegistered)
              // .filter((item) => {
                ...
                <tr className="border h-12 hover:bg-gray-100 ">
                  <td className="pl-5 text-2xl">
                    <div className="flex gap-2">
                      ...
                      <MarkObituaryPopUp  name="activate"
