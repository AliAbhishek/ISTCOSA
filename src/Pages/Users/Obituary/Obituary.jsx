...
function Obituary(props) {
  ...
  const [obituary, setobituaryuser] = useState([]);
  ...
  useEffect(() => {
    getregistereduser("obituary", page).then((res) => setobituaryuser(res));
  }, [page,flag]);
      ...
      <div className="overflow-auto h-[400px] mt-6 border">
        <div className="mx-6 flex flex-col md:flex-row justify-between h-12 text-sm">
          ...
          <div>
            {" "}
            <UserPagination
              pages={obituary[0]?.PageSize}
              changepage={changepage}
              page={page}
              setpage={setpage}
              TotalPageCount={obituary[0]?.TotalPageCount}
              registeredUser={obituary}
            />
          </div>
        </div>
        <table className="w-full  ">
          ...
          <tbody className="overflow-y-scroll no-scrollbar lg:top-0  md:top-14      ">
            {(props.filteredresult[0]?props.filteredresult:obituary)
              // .filter((item) => {
